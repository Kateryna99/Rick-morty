...
const bannerList = [
    ...
    {
        image: isProcessProduction ? '/Rick-Morty/banner/rick_computer.jpg' : '/banner/rick_computer.jpg',
    },
    {
...
async function insertBanner() {
    ...
    for (const item of bannerList) {
        const exists = db.prepare('SELECT COUNT(*) FROM banner WHERE image = ?').get(item.image)['COUNT(*)'];
        if (exists === 0) {
            stmt.run(item);
        }
    }
